...
class Forms {
    ...
    static upload_form_with_preview() {
        $('#update_profil_image').submit();
    }

    static register_new_profil_image() {
        $.ajax({
            url: '/rest/speaker/register_profil_image/id='+$('.user-login-logout-profil').data('id'),
            type: 'post',
            data: {
                image_base64: $('#image').attr('src')
            }
        }).done(data => {
            if(data.success) {
                Auth.get_user_profil()
                ...
                $('#modal_error #error_page_content').html('Votre nouvelle photo de profil n\'à pas pu être enregistrée');
                $('#modal_error').modal('open');
            }
        });
    }

    static onsubmit_upload_form_with_preview() {
        $('#update_profil_image').on('submit', e => {
            e.preventDefault();
            let action = $('#update_profil_image').attr('action');
            let enctype = $('#update_profil_image').attr('enctype');
            let method = $('#update_profil_image').attr('method');

            let formdata = new FormData();
            formdata.append('image', $('#update_profil_image input[type=file]')[0].files[0]);
            ...
            $.ajax({
                ...
                if(data.image_base64 !== '') {
                    $('#image').attr('src', data.image_base64);
                }
                else {
                    $('#image').attr('src', 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAesAAAHrCAQAAABF+n0qAAAABGdBTUEAALGPC/' +
